...
export async function GET(_req: Request, ctx: RouteContext<"/g/[code]/play">) {
  ...
  const group = await db.group.findUnique({
    where: { code },
    include: { workshop: { select: { archived: true, colorPrimary: true } } },
  });
  ...
  const html =
    ...
    `<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{margin:0;min-height:100svh;display:flex;align-items:center;justify-content:center;font-family:system-ui,sans-serif;background:${group.workshop.colorPrimary};color:#fff;text-align:center}div{padding:2rem}h1{font-size:2rem}</style></head><body><div><h1>🎮 Hier entsteht euer Spiel!</h1><p>Sagt der KI unten, was ihr bauen wollt — dann erscheint es hier.</p></div></body></html>`;
